...
__attribute__((section(".kernel_text"))) uint32_t kernel_handler_irq(context_t* context);
...
__attribute__((section(".kernel_text"))) uint32_t kernel_handler_irq(context_t* context)
{
    ...
    */
    ...
    if (task_switch == 1)
    {
        ...
        asm("CPS #0x13");
        asm("LDR SP, %0" : "=m" (tcb_tareas[current_task].SP_SVC_VMA)); // Tengo que guardar el valor del stack pointer SVC que apunta a la tarea siguiente
        asm("CPS #0x12");
    }

    GICC0->EOIR = id; //End of interrupt

    return tcb_tareas[current_task].SP_IRQ_VMA; //Devuelvo el SP
}
